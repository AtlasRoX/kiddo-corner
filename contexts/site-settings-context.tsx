...
export function SiteSettingsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      fetchSettings()
    }, 300) // Increase to 300ms for more reliable hydration
  ...
  const fetchSettings = async (retryCount = 0) => {
    try {
      ...
      try {
        ...
        const { error: pingError } = await supabase.from("site_settings").select("count").limit(1).single()
        if (pingError && pingError.code === "PGRST116") {
          // This is just a "no rows returned" error, which is fine for a connectivity test
        } else if (pingError) {
          throw new Error(`Supabase connectivity error: ${pingError.message}`)
        }
      } catch (connErr) {
        console.warn("Supabase connection check failed, using default settings:", connErr)
        // Don't throw here - continue with defaults
        return
      }
      ...
      // Don't retry too many times, as it might indicate a permanent issue
      if (retryCount < 2) {
        const delay = Math.pow(2, retryCount) * 1000
        console.log(`Retrying fetch settings in ${delay}ms...`)
